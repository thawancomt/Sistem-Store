class Analyze():
    ...
    def generate_analyze(self):
        ...
        if self.get_min() < 0:
            for day in range(len(self.data)):
                if self.data[day] < 0:
                    advices['has_day_with_negative_production'].append(self.dates[day])
        ...
        if texts['resume']:
            ...
            for insight in result.keys():
                ...
                if insight == 'has_day_with_negative_production':
                    insights_resumes.append(f'These days has negatives productions: {result[insight]}')
